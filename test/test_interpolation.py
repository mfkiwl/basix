...
def test_interpolation_matrix(cell_name, order, element_name):
    element = basix.create_element(element_name, cell_name, order, basix.MomentType.DP_equispaced)
    i_m = element.interpolation_matrix
    tabulated = element.tabulate(0, element.points)[0]
    print(max(max(abs(j) for j in i) for i in tabulated))
    ...
    print(np.linalg.norm(coeffs - np.identity(coeffs.shape[0])))
